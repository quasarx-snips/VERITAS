...
def verify_evidence_pair(
    ...
    *,
    ...
    primary_name, primary = select_primary_certificate(certificates, source.enhanced.shape, coverage_config)
    agreement = transform_agreement(certificates, source.enhanced.shape)
    # Correspondence must cover both images.  Source-only occupancy can falsely
    # promote a many-to-one crop or repeated-pattern registration.
    source_coverage = calculate_spatial_coverage(primary.source_inliers, source.enhanced.shape, coverage_config)
    reference_coverage = calculate_spatial_coverage(primary.reference_inliers, reference.enhanced.shape, coverage_config)
    source_entropy = calculate_spatial_entropy(primary.source_inliers, source.enhanced.shape, coverage_config)
    reference_entropy = calculate_spatial_entropy(primary.reference_inliers, reference.enhanced.shape, coverage_config)
    coverage = source_coverage if source_coverage["coverage_ratio"] <= reference_coverage["coverage_ratio"] else reference_coverage
    entropy = source_entropy if source_entropy["normalized_entropy"] <= reference_entropy["normalized_entropy"] else reference_entropy
    ...
    disagreement = summarize_feature_disagreement(quorum)
    disagreement.update({
        "primary_detector": primary_name,
        "transform_agreement": agreement,
        "bidirectional_coverage": {
            "source": source_coverage["coverage_ratio"],
            "reference": reference_coverage["coverage_ratio"],
            "conservative": coverage["coverage_ratio"],
        },
    })
    # A single detector can still produce a useful PARTIAL result, but STRONG
    # requires agreement from another detector when more than one certified map
    # exists.  This turns multi-detector evidence into a real decision input.
    if len(agreement["certified_detectors"]) > 1:
        disagreement["disagreement_score"] = 1.0 - agreement["agreement_strength"]
    counter = CounterEvidence(summarize_residuals(residual_values, primary.threshold), disagreement, summarize_spatial_concentration(coverage, entropy))

    # Phase 2 evidence bundle
    verification_evidence = fuse_evidence(feature_evidence, match_evidence, geometries[primary_name], spatial, quorum, counter)

    # Phase 3: Partial Correspondence Analysis
    partial_correspondence_result = analyze_partial_correspondence(
        match_result=match_results[primary_name],
        certificate=primary,
        image_shape=source.enhanced.shape,
