"""VERITAS preprocessing — image loading, normalisation, and enhancement.

P0.1 — migrated from the reference repository's ``ImagePreprocessor``. The proven
numerical behavior is preserved:

- load grayscale or colour images from disk;
- normalise arbitrary numeric arrays to finite ``uint8`` (BGR/BGRA colour images
  are reduced to a single channel; NaN/±inf intensities are handled);
- apply CLAHE contrast enhancement.

Domain-specific framing from the reference implementation has been removed.

Dependencies: opencv-python, numpy.
"""

from pathlib import Path
from typing import Tuple, Union

import cv2
import numpy as np

ImageArray = np.ndarray


class Preprocessor:
    """Loads grayscale images, normalises them to finite uint8, and applies CLAHE."""

    def __init__(
        self,
        clip_limit: float = 2.5,
        tile_grid_size: Tuple[int, int] = (8, 8),
    ) -> None:
        """Initialise the preprocessor with CLAHE parameters.

        Args:
            clip_limit: CLAHE clipping limit (2.5 is a common default).
            tile_grid_size: size of the CLAHE tile grid.
        """
        self.clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=tile_grid_size)

    def load(self, image_path: Union[str, Path]) -> ImageArray:
        """Load a grayscale image from disk."""
        img = cv2.imread(str(image_path), cv2.IMREAD_GRAYSCALE)
        if img is None:
            raise FileNotFoundError(f"Could not load image at {image_path}")
        return img

    def normalize(self, img: ImageArray) -> ImageArray:
        """Return a finite grayscale uint8 image without mutating the input."""
        array = np.asarray(img)
        if array.size == 0 or array.ndim not in (2, 3):
            raise ValueError("image must be a non-empty grayscale or BGR/BGRA array")
        if array.ndim == 3:
            if array.shape[2] == 3:
                array = cv2.cvtColor(array, cv2.COLOR_BGR2GRAY)
            elif array.shape[2] == 4:
                array = cv2.cvtColor(array, cv2.COLOR_BGRA2GRAY)
            else:
                raise ValueError("colour images must have 3 or 4 channels")
        if array.dtype == np.uint8:
            return array.copy()
        values = array.astype(np.float32, copy=False)
        finite = values[np.isfinite(values)]
        if not len(finite):
            raise ValueError("image contains no finite intensity values")
        lo, hi = float(finite.min()), float(finite.max())
        if hi - lo < 1e-6:
            return np.zeros(values.shape, dtype=np.uint8)
        values = np.nan_to_num(values, nan=lo, posinf=hi, neginf=lo)
        return np.clip((values - lo) * (255.0 / (hi - lo)), 0, 255).astype(np.uint8)

    def enhance(self, img: ImageArray) -> ImageArray:
        """Apply CLAHE contrast enhancement."""
        return self.clahe.apply(img)

    def process(self, image_path: Union[str, Path]) -> Tuple[ImageArray, ImageArray]:
        """Load and preprocess an image.

        Returns:
            Tuple of (normalized_raw, enhanced) grayscale images.
        """
        raw = self.normalize(self.load(image_path))
        enhanced = self.enhance(raw)
        return raw, enhanced