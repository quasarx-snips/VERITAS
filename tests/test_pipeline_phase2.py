...
def test_phase2_pipeline_preserves_three_detector_evidence():
    ...
    assert set(evidence.features) == {"sift", "orb", "akaze"}
    assert evidence.geometry.model == "affine"
    assert evidence.spatial.normalized_entropy is not None
    assert "concentrated" in evidence.counter_evidence.spatial_concentration
